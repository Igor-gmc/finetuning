"""
Fine-tuning Qwen2.5-1.5B-Instruct с использованием LoRA (фиксированная версия)
"""
...
class QwenFineTuner:
    ...
    def load_model(self):
        ...
        if is_macos:
            print(
                f"Загрузка модели {self.model_name} без квантизации (macOS)...",
                flush=True,
            )
        else:
            print(
                f"Загрузка модели {self.model_name} с INT4 квантизацией...", flush=True
            )
    ...
    def prepare_dataset(self, csv_path, system_instruction, max_samples=None):
        ...
        def create_prompt(row):
            messages = [
                ...
                {
                    "role": "user",
                    "content": f"Контекст: {row['context']}\n\nВопрос: {row['question']}",
                },
